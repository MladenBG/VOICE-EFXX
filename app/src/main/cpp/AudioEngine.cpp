...
AudioEngine::AudioEngine() {
    delayBufferL_.resize(48000 * 2, 0.0f); delayBufferR_.resize(48000 * 2, 0.0f);
    modBuffer_.resize(48000 * 2, 0.0f);
...
// Parameter setters
void AudioEngine::setDelayParams(bool e, float tl, float tr, float f, float m) { delayEnabled_=e; delayTimeL_=tl; delayTimeR_=tr; delayFeedback_=f; delayMix_=m; }
...
void AudioEngine::setCompParams(bool e, float t, float r, float a, float rel) { compEnabled_=e; compThresh_=t; compRatio_=r; compAttack_=a; compRelease_=rel; }
...
oboe::DataCallbackResult AudioEngine::onAudioReady(oboe::AudioStream *stream, void *audioData, int32_t numFrames) {
    ...
    processAudio(inBuf, outBuf, result.value());
...
void AudioEngine::processAudio(float* inputBuffer, float* outputBuffer, int32_t numFrames) {
    ...
    float peakAmp = 0.0f; std::vector<int16_t> wavData; if(isRecording_) wavData.reserve(numFrames*2);
    ...
    for (int i = 0; i < numFrames; i++) {
        ...
        if (isRecording_) { wavData.push_back((int16_t)(outL*32767.f)); wavData.push_back((int16_t)(outR*32767.f)); }
    ...
    if (isRecording_ && wavFile_.is_open()) { wavFile_.write(reinterpret_cast<const char*>(wavData.data()), wavData.size()*sizeof(int16_t)); totalFramesWritten_ += (wavData.size()/2); }
}